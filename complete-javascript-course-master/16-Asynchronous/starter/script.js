...
const whereAmI = function (lat, lng) {
  getJSON(
    `https://geocode.xyz/${lat},${lng}?geoit=json&auth=427797822950341702695x55143`
  )
    .then(data => {
...
/* const lotteryPromise = new Promise(function (reolve, reject) {
  console.log('Lotter draw is happening');
  setTimeout(function () {
    if (Math.random() >= 0.5) {
      reolve('You WIN 🎉'); // fulfilled promies
    } else {
      reject(new Error('You lost your money 💩')); // catch
    }
  }, 2000);
});
...
const wait = function (seconds) {
  return new Promise(function (reolve) {
    setTimeout(reolve, seconds * 1000);
  });
};
...
const getPosition = function () {
  return new Promise(function (reolve, reject) {
    ...
    //   position => reolve(position),
    //   err => reject(err)
    // );
    navigator.geolocation.getCurrentPosition(reolve, reject);
  });
};

// getPosition().then(pos => console.log(pos));
// console.log('We do not have to wait! :)');
...
get3Countries('portugal', 'mexico', 'poland'); */
